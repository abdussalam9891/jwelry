...
function startOtpTimer() {
  ...
  otpCountdown = setInterval(() => {
    ...
    timer.textContent = `Resend OTP in ${mins}:${secs}`;
...
function initOtpAuth() {
  ...
  async function sendOtp() {
    ...
    try {
      ...
      showToast(err?.message || "OTP request failed");
    } finally {
  ...
  verifyBtn.onclick = async () => {
    ...
    if (!/^\d{6}$/.test(otp)) {
      return showToast("Enter valid 6-digit OTP");
    }
...
function initForgotModal() {
  ...
  document.getElementById("openForgotModal").onclick = () => {
    modal.classList.remove("hidden");
  };
...
function initForgotPassword() {
  ...
  btn.onclick = async () => {
    try {
      ...
      if (!email) {
        return showToast("Email required");
      }
