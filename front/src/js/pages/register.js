...
import { register, verifyEmailOtp } from "../services/authService.js";
  ...
  form.onsubmit = async (e) => {
    ...
    try {
      ...
      if (!/^[A-Za-z\s]+$/.test(name)) {
        return showToast("Name must contain letters only");
      }

      if (name.length < 2) {
        return showToast("Name too short");
      }
      ...
      if (password.length < 6) {
        return showToast("Min 6 characters required");
      }
