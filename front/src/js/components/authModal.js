...
async function initModal() {
  ...
  try {
    const res = await fetch("/src/htmlComponents/authModal.html");

    if (!res.ok) {
      throw new Error(`Auth modal failed to load: ${res.status}`);
    }
    ...
    if (!modal) {
      throw new Error("authModal not found");
    }
    ...
    document
      .getElementById("goToRegisterBtn")
...
function initOtpAuth() {
  ...
  verifyOtpBtn?.addEventListener("click", async () => {
    try {
      ...
      if (!/^\d{6}$/.test(otp)) {
        return showToast("Enter valid OTP");
      }
      ...
      const res = await fetch(`${CONFIG.API_BASE}/v1/auth/firebase-login`, {
        method: "POST",
        headers: {
