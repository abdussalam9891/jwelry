...
const {
  ...
  const shopLinks = jewelleryNavigation
    .map(
      (cat) => `
      <li>
        <a href="/pages/products.html?category=${cat.category}" class="hover:text-[var(--color-accent-gold)] transition-colors">
          ${cat.label}
        </a>
      </li>`
    )
  ...
  const collectionLinks = collections
    ...
    .map(
      (col) => `
      <li>
        <a href="/pages/collection.html?slug=${col.slug}" class="hover:text-[var(--color-accent-gold)] transition-colors">
          ${col.name}
        </a>
      </li>`
    )
  ...
  const popularSearches = jewelleryNavigation
    .flatMap((cat) =>
      ...
      <a href="/pages/products.html?category=${cat.category}&style=${style.value}"
        class="text-white/50 hover:text-[var(--color-accent-gold)] transition-colors">
  ...
  const footerHTML = `
  <footer class="bg-[var(--color-brand-dark)] text-white pt-14 pb-8">
...
>
          ...
          type="email"
            ...
            class="flex-1 bg-white/5 border border-white/25 focus:border-[var(--color-accent-gold)] outline-none text-sm px-4 py-2.5 placeholder:text-white/40 transition-colors"
          />
          <button
          id="newsletterBtn"
            type="submit"
            class="border border-l-0 border-white/25 px-5 text-xs font-semibold uppercase tracking-wide hover:bg-[var(--color-accent-gold)] hover:border-[var(--color-accent-gold)] hover:text-[var(--color-brand-dark)] transition-colors"
          >
            Join
          </button>
        </form>
  ...
  rel="noopener noreferrer"
              aria-label="Visit Gemora on Instagram" class="text-white/70 hover:text-[var(--color-accent-gold)] transition-colors">
            ...
            <a href="https://pinterest.com/gemorajewellry/" target="_blank" rel="noopener noreferrer"
              aria-label="Visit Gemora on Pinterest" class="text-white/70 hover:text-[var(--color-accent-gold)] transition-colors">
              <svg class="w-5 h-5" fill="currentColor" aria-hidden="true" viewBox="0 0 24 24">
  ...
  >
    ...
    />
            ...
            <ul class="space-y-2.5 text-sm text-white/70 mt-3 lg:mt-0 lg:pointer-events-auto">
              ${shopLinks}
              <li>
                <a href="/pages/products.html" class="text-[var(--color-accent-gold)] font-medium hover:text-white transition-colors">View All →</a>
              </li>
            </ul>
          </details>
        </div>
...
>
  ...
  >
    ...
    />
            ...
            <ul class="space-y-2.5 text-sm text-white/70 mt-3 lg:mt-0 lg:pointer-events-auto">
              <li><a href="/pages/about.html" class="hover:text-[var(--color-accent-gold)] transition-colors">About Us</a></li>
              <li><a href="/pages/contact.html" class="hover:text-[var(--color-accent-gold)] transition-colors">Contact</a></li>
              <li><a href="/pages/shipping.html" class="hover:text-[var(--color-accent-gold)] transition-colors">Shipping</a></li>
              <li><a href="/pages/returns.html" class="hover:text-[var(--color-accent-gold)] transition-colors">Returns</a></li>
              <li><a href="/pages/warranty.html" class="hover:text-[var(--color-accent-gold)] transition-colors">Warranty</a></li>
            </ul>
          </details>
          ...
          <div class="mt-5 pt-5 border-t border-white/15 lg:border-none lg:pt-0 lg:mt-6 text-sm text-white/70 space-y-1.5">
            ${address ? `<p>${address}</p>` : ""}
            <p>
              <a href="${email ? `mailto:${email}` : "#"}" class="hover:text-[var(--color-accent-gold)] transition-colors">
                ${email || "Not Available"}
              </a>
            </p>
            <p>
              <a href="${phone ? `tel:${phone}` : "#"}" class="hover:text-[var(--color-accent-gold)] transition-colors">
                ${phone || "Not Available"}
              </a>
            </p>
          </div>
